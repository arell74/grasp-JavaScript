// const coba = ['satu', 'dua', 'tiga'];

// const [a, b, c] = coba;
// console.log(a);
// console.log(b);
// console.log(c);

// // destructuring array
// const arr = ['halo', 'nama', 'saya', 'clorinde'];
// // const [perkenalan, satu, dua, nama] = arr;

// //skipping items
// const [perkenalan, , , nama] = arr;
// console.log(nama);

//swap items
// let a = 1;
// let b = 2;
// console.log(a);
// console.log(b);
// [a,b] = [b,a];
// console.log(a);
// console.log(b);

// return value pada function
// function coba() {
//     return [1, 2];
// }

// const [a, b] = coba();
// console.log(a);

// // rest parameter 
// const [a, ...values] = [1, 2, 3, 4];
// console.log(a);
// console.log(values);

// // desctructuring in object
// const mhs = {
//     nama: 'raiden',
//     umur: 20,
//     email: 'raiden@gmail.com'
// };

// const {nama, umur, email} = mhs;
// console.log(`Nama saya ${nama}, umur: ${umur}, email: ${email}`);

// // assigment tanpa deklarasi objek
// ({nama, umur} = {
//     nama: 'raiden',
//     umur: 20
// });
// console.log(`${nama} and ${20}`);

// // assign ke variable baru
// const mhs = {
//     nama: 'zani',
//     umur: 20
// };

// const {nama: n, umur: u} = mhs;
// console.log(n);

// // memberikan default value
// const mhs = {
//     nama: 'zani',
//     umur: 20,
//     email: 'zani@gmail.com',
// };

// const { nama, umur, email = 'email@default.com' } = mhs;
// console.log(email);

// memberi nilai default + assign ke variable baru
// const mhs = {
//     nama: 'zani',
//     umur: 20,
//     email: 'zani@gmail.com'
// };

// const { nama: n, umur: u, email: e = 'default@gmail.com'} = mhs;
// console.log(e);

// // rest Parameter
// const mhs = {
//     nama: 'zani',
//     umur: 20,
//     email: 'zani@gmail.com',
// };

// const {nama, ...value} = mhs;
// console.log(value);

// mengambil field pada object, setelah dikirim sebagai parameter untuk function
const mhs = {
    id: 123,
    nama: 'raiden',
    umur: 20,
    email: 'raiden@gmail.com',
};

function getIdMhs({ id }) {
    return id
}

console.log(getIdMhs(mhs));

